...
- 当前实现固定浅色主题，功能色使用 [`03-functional-color-brightness-comparison.html`](../prototypes/03-functional-color-brightness-comparison.html) 的浅色 B 档；深色/跟随系统和动态配色延后到视觉基线成熟后单独设计，不得在本阶段引入兼容入口。
...
实现与三个 HTML 原型一致的可安装产品壳：Compact 窗口使用“首页 / 视频 / 整理”三项底栏，Medium/Expanded 窗口切换导航轨，播放页隐藏一级导航；处理中心和设置保留在顶部；当前固定浅色主题与纯黑播放画布使用单一 Token 事实源。
...
- 主题当前固定为浅色：由 `ThemeRepository` 提供浅色设置状态，不保留动态配色、深色切换或旧主题偏好兼容分支；待视觉基线成熟后再单独设计主题扩展。
...
* **任务 2.2: 固定浅色主题与系统栏映射** `[并行]`
  * **依赖**: 任务 2.1
  * **TDD 循环**: Red：先测试非浅色设置不会进入当前首页；Green：实现浅色 ThemeRepository 与系统栏映射；Refactor：分离应用表面和播放器画布，为后续主题扩展保留清晰边界。
  * **测试预期**:
    * 正常路径: 首页固定使用浅色 B 档和浅色系统栏图标；播放画布始终纯黑并使用独立控制层映射。
    * 异常路径: 不存在的主题偏好不会导致启动失败；Material You 和深色入口不出现在当前设置界面。
  * **DoD**: JVM 状态测试与 Compose 主题测试通过；浅色系统栏映射完成；深色主题需求单独立项，不在本任务中伪实现。
...
交付“整理”一级页的收藏、标签、播放列表、智能集合和最近整理，建立标签组、多种集合和播放历史；让首页以统计卡片、继续观看、最近添加、我的合集、常用文件夹和需要处理构成可配置的高频入口。首页使用有限数据库投影，不持有全量媒体列表。
...
- `HistoryEntry`、`RecentlyOrganizedEntry`、`ContinueWatchingPolicy`、`HomeCardLayout`：播放次数、最近播放、整理时间、继续观看、首页卡片顺序/隐藏状态和有限摘要投影。
...
* **任务 6.6: 实现首页卡片编排、有限投影与空状态** `[并行]`
  * **依赖**: 6.3、6.5、3.9
  * **TDD 循环**: Red：覆盖卡片无数据、顺序/隐藏配置、有限查询、点击恢复和刷新；Green：实现 `HomeCardLayout`、卡片编排与数据库投影；Refactor：删除全量媒体 Flow，卡片只接收只读投影。
  * **测试预期**:
    * 正常路径: 首页标题为“影里”，默认显示统计卡片、继续观看、最近添加、我的合集、常用文件夹和需要处理；用户可在 Bottom Sheet 中拖拽排序、隐藏/显示并恢复默认；点击进入统计、播放、视频或整理上下文正确。
    * 异常路径: 某卡片查询失败只局部降级；空卡片不占位；全部隐藏时可恢复；首页不重新加载全量媒体；刷新可取消；搜索仍不得提供联网或建议内容。
  * **DoD**: 首页状态矩阵、有限查询、布局持久化、导航契约和无障碍测试通过；首屏只加载有限投影。
